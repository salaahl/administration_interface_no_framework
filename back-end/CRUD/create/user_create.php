...
$partner = $db->query(
  ...
  drinks_permission TINYINT(1) DEFAULT 0,
  newsletter_permission TINYINT(1) DEFAULT 0,
  planning_permission TINYINT(1) DEFAULT 0,
  PRIMARY KEY (mail),
  FOREIGN KEY (brand) REFERENCES brand(name)
  )"
);
...
$structure = $db->query(
  ...
  PRIMARY KEY (mail),
  FOREIGN KEY (brand) REFERENCES brand(name),
  FOREIGN KEY (city) REFERENCES partenaire(city)
  )"
);
