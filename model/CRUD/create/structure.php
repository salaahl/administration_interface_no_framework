...
if (isset($_POST["structure_new"])) {
  ...
  if ($result == false) {
    ...
    if (mysqli_affected_rows($db) > 0) {
      ...
      $mail_partner = $db->prepare(
        "SELECT mail
          FROM partner
          WHERE 
          city = ?"
      );
      $mail_partner->bind_param("s", $city);
      $mail_partner->execute();
      $mail_partner->store_result();
      $mail_partner->bind_result($partner_mail);
      $mail_partner->fetch();
      $mail_partner->close();
  ...
  <body>
    <table>
      ...
      $mail_cc = $_SERVER["SERVER_NAME"] == "localhost" ? getenv("EMAIL_CC") : $partner_mail;
